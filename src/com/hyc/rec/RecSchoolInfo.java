...
public class RecSchoolInfo {
	...
	public void receiveSchInfo() {
		...
		try {
			...
			Log.e("RecSchoolInfo", "返回码" + five_urlConnection.getResponseCode());
			...
			if (five_urlConnection.getResponseCode() == 200) {
				...
				String line;
				System.out.println(" 141414141414141414141 ");
				...
				Log.e("RecSchoolInfo", "city:" + city);
				Log.e("RecSchoolInfo", "district:" + district);
				Log.e("RecSchoolInfo", "mobile:" + mobile);
				Log.e("RecSchoolInfo", "content:" + content);
				Log.e("RecSchoolInfo", "province:" + province);

				Log.e("RecSchoolInfo", "学校信息：" + logoId + schoolName + qq
	...
	public void receiveSchoolInfoMain(Handler mHandler, String macAddress,
			Context mContext) {

		URL five_url;
		try {
			five_url = new URL(
					"http://api.360baige.cn/company/info?accesstoken="
							+ macAddress);
			HttpURLConnection five_urlConnection = (HttpURLConnection) five_url
					.openConnection();
			...
			if (five_urlConnection.getResponseCode() == 200) {
				...
				try {
					jsonNow = new JSONObject(main_String.substring(4))
							.getJSONObject("company");
					errcode = new JSONObject(main_String.substring(4))
							.getString("errcode");
					if (!errcode.equals("0")) {
				...
				main_String = null;
				if (schoolName != null) {
					if (!schoolName.equals(dbcompany.queryName())
							|| dbcompany.queryName() == null) {
	...
	private boolean getDate(String macAddress, Handler mHandler, Context context) {
		...
		try {
			...
			timeUrl = new URL(
					"http://api.360baige.cn/equipment/geteqcurrentdate?accesstoken="
							+ macAddress);
